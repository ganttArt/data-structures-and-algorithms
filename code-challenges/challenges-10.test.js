...
const count = (target, input) => {
  ...
  return input.reduce((prev, curr) => prev.concat(curr)).filter(number => number === target).length;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    newArr.push(nestedArray.filter(number => ((number % 5 === 0) && Number.isInteger(number))).map(number => Math.pow(2, number)));
  }
  return newArr;

  // return input.reduce((prev, curr) => prev.concat(curr)).filter(number => ((number % 5 === 0) && Number.isInteger(number))).map(number => Math.pow(2, number));
};
// return input.reduce((prev, curr) => prev.concat(curr)).filter(number => number === target).length;
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5 
...
let findMaleAndFemale = (data) => {
  return data.filter(person => ((person.gender === 'female') || (person.gender === 'male'))).map(person => person.name).join(' and ');
};
...
let findShortest = (data) => {
  let name = data.map(person => person.name).reduce((answerSoFar, value) => {
    answerSoFar = answerSoFar.length >= value.length ? value : answerSoFar;
    return answerSoFar;
  }, 'incrediblylongnamethatisnotgoingtobeoutdone');
  return name;
};
