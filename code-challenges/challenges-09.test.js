...
function transformToLis(obj){
  ...
  for (let [key, value] of Object.entries(obj)){
    tagList.push(`<li>${key}: ${value}</li>`)
  }
  return tagList;
}
// Help from https://stackoverflow.com/questions/684672/how-do-i-loop-through-or-enumerate-a-javascript-object
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const addPurchases = (arr) => {
  let total = arr.reduce((answerSoFar, value) => {
    answerSoFar += value.purchasePrice;
    return answerSoFar;
  }, 0);
  return total;
};
...
const countNumberOfElements = (arr) => {
  let len = arr.reduce((answerSoFar) => {
    answerSoFar += 1;
    return answerSoFar;
  }, 0);
  return len;
...
const reversedString = (str) => {
  ...
  let reversed = strList.reduce((answerSoFar, value) => {
    answerSoFar = value.concat(answerSoFar);
    return answerSoFar;
  }, '');
  return reversed;
};
