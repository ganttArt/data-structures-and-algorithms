...
const validateEmail = (email) => {
  const condition1 = /[^\s.]+@/;
  const condition2 = /[^\s.]+.[^\s.]+@/;
  const condition3 = /[a-zA-Z]+\.[a-zA-Z]+\.[a-zA-Z]+/;
  const condition4 = /.*\.com$|.*\.org$|.*\.net$/;

  return condition1.test(email) &&
    condition2.test(email) &&
    !(condition3.test(email)) &&
    condition4.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  const c1 = /\(\d{3}\) \d{3}-\d{4}/;
  const c2 = /\(\d{3}\)\d{3} \d{4}/;
  const c3 = /\d{3} \d{3}-\d{4}/;
  const c4 = /\d{3}-\d{7}/;
  const c5 = /\d{3}-\d{3} \d{4}/;
  const c6 = /\d{3}-\d{3}-\d{4}/;
  const c7 = /\d{3} \d{3} \d{4}/;
  const c8 = /\d{6}-\d{4}/;
  const c9 = /\d{10}/;
  const c10 = /\d{3} \d{7}/;

  return c1.test(phoneNumber) ||
    c2.test(phoneNumber) ||
    c3.test(phoneNumber) ||
    c4.test(phoneNumber) ||
    c5.test(phoneNumber) ||
...
xdescribe('Testing challenge 6 - unacceptable formats', () => {
